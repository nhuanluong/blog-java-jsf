...
public class MenuService extends AbstractBaseService<Menu, Long> {
    ...
    public Menu save(Menu entity) {
        if (StringUtils.isBlank(entity.getSlug())) {
            ...
            SlugUtil.Friendly<Menu> callableSlug = (id, name) -> {
                BooleanBuilder builder = new BooleanBuilder(qMenu.categorySlug.containsIgnoreCase(name));
                if (id != null && id > 0) {
            ...
            entity.setSlug(SlugUtil.makeSlug(entity.getId(), entity.getName(), callableSlug));
        }
